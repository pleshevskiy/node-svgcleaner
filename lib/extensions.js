/*!
 * node-svgcleaner: lib/extensions.js
 */
...
/**
 * Get a human readable description of where node-svgcleaner is running to support
 ...
 *
 * @return {Array} The currently installed node-svgcleaner bindings
 *
 * @api public
 */
 ...
 * @return {Boolean} True, if node-svgcleaner supports the current platform, false otherwise
 *
 * @api public
 */
...
/**
 * Get binary name.
 * If environment variable SVGCLEANER_BINARY_NAME,
 * .npmrc variable svgcleaner_binary_name or
 * process argument --svgcleaner-binary-name is provided,
...
function getBinaryName() {
    ...
    if (getArgument('--svgcleaner-binary-name')) {
        binaryName = getArgument('--svgcleaner-binary-name');
    } else if (process.env.SVGCLEANER_BINARY_NAME) {
        binaryName = process.env.SVGCLEANER_BINARY_NAME;
    } else if (process.env.npm_config_svgcleaner_binary_name) {
        binaryName = process.env.npm_config_svgcleaner_binary_name;
    } else if (pkg.nodeSvgCleanerConfig && pkg.nodeSvgCleanerConfig.binaryName) {
        binaryName = pkg.nodeSvgCleanerConfig.binaryName;
    } else {
 ...
 * By default fetch from the node-svgcleaner distribution
 * site on GitHub.
 *
 * The default URL can be overriden using
 * the environment variable SVGCLEANER_BINARY_SITE,
 * .npmrc variable svgcleaner_binary_site or
 * or a command line option --svgcleaner-binary-site:
 *
 *   node scripts/install.js --svgcleaner-binary-site http://example.com/
 *
 * The URL should to the mirror of the repository
 * laid out as follows:
 *
 * SVGCLEANER_BINARY_SITE/
 *
 *  v3.0.0
...
function getBinaryUrl() {
    var site = getArgument('--svgcleaner-binary-site') ||
        process.env.SVGCLEANER_BINARY_SITE ||
        process.env.npm_config_svgcleaner_binary_site ||
        (pkg.nodeSvgCleanerConfig && pkg.nodeSvgCleanerConfig.binarySite) ||
        'https://github.com/pleshevskiy/node-svgcleaner/releases/download';
...
/**
 * Get binary dir.
 * If environment variable SVGCLEANER_BINARY_DIR,
 * .npmrc variable SVGCLEANER_BINARY_DIR or
 * process argument --svgcleaner-binary-dir is provided,
...
function getBinaryDir() {
    ...
    if (getArgument('--svgcleaner-binary-dir')) {
        binaryDir = getArgument('--svgcleaner-binary-dir');
    } else if (process.env.SVGCLEANER_BINARY_DIR) {
        binaryDir = process.env.SVGCLEANER_BINARY_DIR;
    } else if (process.env.npm_config_SVGCLEANER_BINARY_DIR) {
        binaryDir = process.env.npm_config_SVGCLEANER_BINARY_DIR;
    } else if (pkg.nodeSvgCleanerConfig && pkg.nodeSvgCleanerConfig.binaryDir) {
        binaryDir = pkg.nodeSvgCleanerConfig.binaryDir;
    } else {
...
/**
 * Get binary path.
 * If environment variable SVGCLEANER_BINARY_PATH,
 * .npmrc variable SVGCLEANER_BINARY_PATH or
 * process argument --svgcleaner-binary-path is provided,
 * select it by appending binary name, otherwise
...
function getBinaryPath() {
    ...
    if (getArgument('--svgcleaner-binary-path')) {
        binaryPath = getArgument('--svgcleaner-binary-path');
    } else if (process.env.SVGCLEANER_BINARY_PATH) {
        binaryPath = process.env.SVGCLEANER_BINARY_PATH;
    } else if (process.env.npm_config_SVGCLEANER_BINARY_PATH) {
        binaryPath = process.env.npm_config_SVGCLEANER_BINARY_PATH;
    } else if (pkg.nodeSvgCleanearConfig && pkg.nodeSvgCleanearConfig.binaryPath) {
        binaryPath = pkg.nodeSvgCleanearConfig.binaryPath;
    } else {
...
function getVersionInfo(binding) {
    return [
        ['node-svgcleaner', pkg.version, '(Wrapper)', '[JavaScript]'].join('\t'),
